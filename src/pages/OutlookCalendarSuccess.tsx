...
const OutlookCalendarSuccess = () => {
  ...
  useEffect(() => {
    if (!error) {
      try {
        window.opener?.postMessage("outlook-calendar-connected", "*");
      } catch (e) {
        // ignore
      }
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-muted/30">
      <div className="bg-card rounded-xl p-10 shadow-lg text-center max-w-sm">
        <svg className="mx-auto mb-4" width="64" height="64" viewBox="0 0 24 24" fill="none">
          <circle cx="12" cy="12" r="11" fill="#dbeafe" stroke="#2563eb" strokeWidth="2" />
          <path d="M7 12.5l3 3 7-7" stroke="#2563eb" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
        <h1 className="text-xl font-semibold text-blue-600 mb-2">¡Conexión exitosa!</h1>
        <p className="text-muted-foreground text-sm leading-relaxed">
          Tu cuenta de Microsoft ha sido vinculada correctamente.
          <br />
...
export default OutlookCalendarSuccess;
