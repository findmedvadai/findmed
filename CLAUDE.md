...
| Hosting | Lovable (`https://findmed.lovable.app` es el `APP_URL` por defecto en Edge Functions) |
...
| `notification_type` | `appointment_scheduled`, `appointment_cancelled_by_patient`, `appointment_cancelled_by_doctor`, `appointment_auto_cancelled`, `appointment_completed`, `postconsultation_submitted` |
...
| `doctors` | Perfil del doctor: `full_name`, `phone`, `address`, `city_id`, `zone_id`, flags `is_active`/`is_deleted`, refs de Google Calendar (`google_refresh_token_ref`, `google_calendar_id`, `google_calendar_connected`) y Outlook (`outlook_refresh_token_ref`, `outlook_calendar_id`, `outlook_calendar_connected`). |
| `doctor_specialties` | M:N doctor ↔ specialty. |
| `doctor_schedule_settings` | 1:1 con `doctors`. `appointment_duration_minutes` (default 30), `min_confirm_hours_before` (default 24), `timezone` (default `America/Mexico_City`). |
| `doctor_weekly_availability` | Horarios semanales `weekday` (0–6) + `start_time`/`end_time` + `is_enabled`. Unique `(doctor_id, weekday)`. |
...
| `patients` | Pacientes identificados por `phone` único + `full_name`. |
| `reservation_sessions` | Token efímero (72 h) que conecta un paciente con un doctor para completar su reserva. Lo crea `triage-webhook`. FK a `patients` y `doctors`. |
| `appointments` | Cita: `doctor_id`, `patient_id`, `start_at`, `end_at`, `status`, `cancel_reason`, `symptoms`, `doctor_notes`, `google_event_id`, `outlook_event_id`, `created_from_session_id`. |
| `appointment_manage_tokens` | Tokens para que el paciente gestione su cita (confirmar/cancelar/reagendar). Expiran al finalizar la cita. |
...
| `APP_URL` | URL pública de la app (default `https://findmed.lovable.app`); se usa para construir links `/reserva?token=…` y `/gestionar?token=…`. |
...
8. Tras cada cambio funcional **verificar que el build pasa**: `npm run build` (o `bun run build`) y `npm run lint`. Si el cambio toca tipos o RLS, correr además `npm run test`.
